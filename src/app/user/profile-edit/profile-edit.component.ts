  ...
  getUserDetails() {
    ...
    this.http.get(this.serviceUrl + "/getuserdetails/" + this.userId)
      .subscribe(function (response) {
       ...
       // console.log(current.ph1+" "+current.ph2+" "+current.ph3);
       current.userData.phone = current.ph1+current.ph2+current.ph3;
       console.log(current.userData.phone);
      },
      function (err) {
  ...
  update(model: any) {
    ...
    this.http.post(this.serviceUrl + "/updateuser", this.modelData)
      .subscribe(function (response) {
        ...
        current.alertMessage = {
          type: 'success',
          title: 'Successfully added Guest Registration',
          data: ''
        };
        var user = JSON.parse(sessionStorage.getItem('user'));
